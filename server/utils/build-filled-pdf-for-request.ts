...
import { request, requestTemplate, requestTemplateValues, roles, signatureFlow, signatures, userRoles } from '~~/lib/db/schema';
...
export async function buildFilledPdfBytesForRequest(
  ...
  const incrementalFieldInstanceIdSet = new Set(
    (options?.fieldInstanceIdFilter ?? [])
      .map(id => String(id ?? '').trim())
      .filter(id => id.length > 0),
  );

  function shouldResolveAutoGeneratedFallback(field: any): boolean {
    if (!isIncrementalMode) {
      // Full rebuild is used by request creation/refresh and must never prefill
      // unresolved auto values that belong to future signing steps.
      return false;
    }
    ...
    return incrementalFieldInstanceIdSet.has(instanceId);
  }

  function resolveFieldInputValue(field: any): string {
    const rawManualValue = getStoredFieldValue(fieldValuesMap, field);
    const manualFieldValue = applyFieldCharacterLimit(rawManualValue, field).trim();
    const formattedManualValue = applyFieldCharacterLimit(formatManualDateTimeValue(field, String(rawManualValue ?? '')), field).trim();
    ...
    if (isAutoGeneratedDateOrTimeField(field)) {
      ...
      if (shouldResolveAutoGeneratedFallback(field)) {
    ...
    if (isDateOrTimeField(field)) {
      return formattedManualValue || manualFieldValue;
    }
