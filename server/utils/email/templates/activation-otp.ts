...
export function activationOtpTemplate(options: {
  to: string;
  name: string;
  otp: string;
}): EmailPayload {
  return {
    to: options.to,
    subject: `รหัสยืนยันการเปิดใช้งานบัญชี`,
    text: [
      `เรียน ${options.name}`,
      `รหัสยืนยันของคุณคือ ${options.otp}`,
    ...
    <table width="560" cellpadding="0" cellspacing="0"
      ...
      <tr>
        <td style="padding:24px 32px;border-bottom:1px solid #e0e0e0;">
          <p style="margin:0 0 2px;font-size:12px;color:#888;">เปิดใช้งานบัญชี</p>
      ...
      <tr>
        <td style="padding:24px 32px;">
          ...
          <p style="margin:0 0 24px;color:#333;">กรุณาใช้รหัสด้านล่างเพื่อยืนยันและเปิดใช้งานบัญชีของคุณ</p>
