...
function buildDateFormatConfig(body: any, fieldType: string): Record<string, unknown> {
  const typeLower = fieldType.toLowerCase();
  if (typeLower !== 'date' && typeLower !== 'time' && typeLower !== 'datetime') {
    return {};
  }
  const config = getAutoDateTimeFormatConfig(body);
  if (typeLower === 'time') {
    return {
...
export default defineEventHandler(async (event: H3Event) => {
  ...
  try {
    ...
    const isCheckboxGroup = normalizedType === 'checkbox' && amount > 1;
    ...
    const isDropdownField = normalizedType === 'dropdown';
    const sessionField = isDropdownField ? null : normalizeSessionField(body.sessionField ?? body.session_field);
    const dropdownConfig = isDropdownField
      ? serializeDropdownConfig(body.dropdownConfig ?? body.dropdown_config)
      : null;

    if (isDropdownField && !dropdownConfig) {
      throw createError({
        statusCode: 400,
        message: 'กรุณาเลือกตารางและคอลัมน์สำหรับ Dropdown',
      });
    }
    ...
    const [newField] = await db.insert(requestTemplateFields).values({
      ...
      isAutoGenerated: normalizedType === 'time'
        ? true
        : (body.isAutoGenerated ?? false),
      dateFormatConfig: buildDateFormatConfig(body, body.type),
    }).returning();
    ...
    const mappedField = {
      ...
      dropdownConfig: serializeDropdownConfig(newField.dropdownConfig),
      dropdown_config: serializeDropdownConfig(newField.dropdownConfig),
      ...
      ...(newField.dateFormatConfig && typeof newField.dateFormatConfig === 'object' ? newField.dateFormatConfig : {}),
    };
