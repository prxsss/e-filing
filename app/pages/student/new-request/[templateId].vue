...
function getSessionFieldBinding(field: any): SessionFieldBinding | null {
  const rawValue = String(field?.sessionField ?? field?.session_field ?? '').trim();
  if (rawValue === 'studentName'
    || rawValue === 'studentId'
  ...
  const fallbackName = String(field?.name ?? field?.label ?? '').trim().toLowerCase();
...
function resolveSessionBoundFieldValue(field: any): string {
  ...
  if (binding === 'studentName') {
    return String(user.value?.fullNameTh ?? user.value?.fullNameEn ?? '').trim();
  }

  if (binding === 'facultyNameTh') {
    return String(user.value?.facultyNameTh ?? '').trim();
  }

  if (binding === 'departmentNameTh') {
    return String(user.value?.departmentNameTh ?? '').trim();
  }

  if (binding === 'departmentCode') {
    return String(user.value?.departmentCode ?? '').trim();
  }

  if (binding === 'studentYearCurrent') {
  ...
  return String(user.value?.studentId ?? '').trim();
}
...
function hydrateFieldValueKeys() {
  for (const field of placedFields.value) {
    const valueKey = getFieldValueKey(field);
    const manualKey = getManualFieldKey(field);
    const sessionBoundValue = resolveSessionBoundFieldValue(field);

    if (valueKey && sessionBoundValue.length > 0) {
      fieldValues.value[valueKey] = sessionBoundValue;
      continue;
    }
...
function resolveCurrentFieldValue(field: any): string {
  ...
  const sessionBoundValue = resolveSessionBoundFieldValue(field);
  if (sessionBoundValue.length > 0) {
    return sessionBoundValue;
  }
...
function getFieldSectionTitle(field: any): string {
  return String(field?.formSectionTitle ?? '').trim() || 'Request Information';
}

type FormSectionItem
  = | { kind: 'field'; field: any }
    | { kind: 'checkboxGroup'; groupId: string; fields: any[] };
...
const visibleFormSections = computed<FormSection[]>(() => {
  const sections: FormSection[] = [];
  let currentSection: FormSection | null = null;
  let checkboxGroupIndexes = new Map<string, number>();
  ...
  for (const field of orderedVisibleFillableFields.value) {
    ...
    if (!currentSection || currentSection.title !== sectionTitle) {
      ...
      checkboxGroupIndexes = new Map<string, number>();
    }

    if (isCheckboxField(field)) {
      const groupId = getCheckboxGroupId(field);
      if (groupId.length > 0) {
        const existingIndex = checkboxGroupIndexes.get(groupId);
        if (existingIndex !== undefined) {
          const existingItem = currentSection.items[existingIndex];
          if (existingItem && existingItem.kind === 'checkboxGroup') {
            existingItem.fields.push(field);
          }
        }
        else {
          checkboxGroupIndexes.set(groupId, currentSection.items.length);
    ...
    currentSection.items.push({ kind: 'field', field });
  }
...
function hasCheckboxGroupValidationError(fields: any[]): boolean {
  return fields.some(field => hasFieldValidationError(field));
}

function isCheckboxGroupRequired(fields: any[]): boolean {
  return fields.some(field => isFormFieldRequired(field));
}

function getCheckboxGroupContainerId(fields: any[]): string {
  const firstField = fields[0];
  return firstField ? getFieldContainerId(firstField) : '';
}
                  ...
                  >
                    <div :class="itemIndex > 0 ? 'border-t border-gray-100 pt-3' : ''">
                      ...
                      >
                        ...
                        >
                          {{ getFieldValidationMessage(item.field) }}
                        </p>
                      </div>

                      <div
                        v-else
                        :id="getCheckboxGroupContainerId(item.fields)"
