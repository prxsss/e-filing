...
function buildManualFieldValuesPayload(): FieldValue[] {
  const allowedFieldIds = new Set(
    placedFields.value
      .filter(field => !isAutoGeneratedField(field))
      .map(field => Number.parseInt(String(field.id ?? ''), 10))
      .filter(fieldId => Number.isFinite(fieldId)),
  );

  return Object.entries(fieldValues.value)
    .map(([fieldId, value]) => ({
      fieldId: Number.parseInt(fieldId, 10),
      value: value || '',
    }))
    .filter(item => Number.isFinite(item.fieldId) && allowedFieldIds.has(item.fieldId));
}
...
const studentFields = computed(() => {
  return fillableFields.value.filter((f: any) => !isAutoGeneratedField(f));
});
...
async function refreshPreviewPdf() {
  ...
  const fieldsForPreview = fillableFields.value;
