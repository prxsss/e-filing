...
// Fetch user data
const { data: userData } = await useFetch<User>(`/api/users/${userId}`);
...
watch(userData, (newData) => {
  if (newData) {
    form.value = {
      ...
      email: newData.email || '',
      facultyId: newData.facultyId || null,
...
async function handleUpdateUser(event: FormSubmitEvent<Schema>) {
  try {
...
<template>
  <div class="flex flex-col relative">
    <!-- Main Content -->
    <UForm :schema="schema" :state="form" class="flex-1 max-w-360 mx-auto w-full space-y-6" @submit.prevent="handleUpdateUser">
      <UButton
        type="button"
