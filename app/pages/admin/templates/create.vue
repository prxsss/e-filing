...
import { formatAutoGeneratedDate, getAutoDateTimeFormatConfig, parseDateInputToBangkokDate } from '~~/shared/auto-date-time-format';
...
const previewDateInputs = ref<Record<string, string>>({});
...
function getPreviewRequestValue(field: FieldInstance): string {
  const key = getPreviewFieldKey(field);
  if (!key) {
    return '';
  }
  ...
  if (fieldType === 'date' && !isAutoGeneratedDateOrTimeField(field)) {
    return previewDateInputs.value[key] || '';
  }

  return previewFieldValues.value[key] || '';
}

function getPreviewOverlayValue(field: FieldInstance, requestValue: string): string {
  const fieldType = getFieldType(field);
  if (fieldType === 'date' && !isAutoGeneratedDateOrTimeField(field)) {
    const parsed = parseDateInputToBangkokDate(requestValue);
    return parsed ? formatAutoGeneratedDate(field, parsed) : '';
  }

  return requestValue;
}

function buildPreviewSnapshots(): { requestValues: Record<string, string>; overlayValues: Record<string, string> } {
  ...
  for (const field of placedFields.value) {
    ...
    const requestValue = getPreviewRequestValue(field);
    requestValues[key] = requestValue;
    overlayValues[key] = getPreviewOverlayValue(field, requestValue);
  }

  return { requestValues, overlayValues };
}
...
function getBangkokTodayDateInputValue(): string {
  return new Date().toLocaleDateString('en-CA', { timeZone: 'Asia/Bangkok', calendar: 'gregory' });
}
...
const isPreviewDateField = computed<boolean>(() => {
  if (!selectedField.value || fileType.value !== 'pdf') {
    return false;
  }

  const fieldType = getFieldType(selectedField.value);
  return fieldType === 'date' && !isAutoGeneratedDateOrTimeField(selectedField.value);
});

const selectedFieldPreviewDateInput = computed<string>({
  get: () => {
    const key = getPreviewFieldKey(selectedField.value);
    return key ? (previewDateInputs.value[key] || '') : '';
  },
  set: (value) => {
    const key = getPreviewFieldKey(selectedField.value);
    if (!key) {
    ...
    if (value) {
      previewDateInputs.value[key] = value;
    }
    else {
      delete previewDateInputs.value[key];
    }
  },
});

function setPreviewDateValue(rawValue: string, field: FieldInstance): void {
  const key = getPreviewFieldKey(field);
  if (!key) {
    return;
  }
  ...
  previewDateInputs.value[key] = rawValue;
  const parsed = parseDateInputToBangkokDate(rawValue);
  const formatted = parsed ? formatAutoGeneratedDate(field, parsed) : '';

  if (formatted) {
    previewFieldValues.value[key] = formatted;
  }
  else {
    delete previewFieldValues.value[key];
    delete previewSyncedFieldValues.value[key];
...
function ensurePreviewDatePrefill(field: FieldInstance | null): void {
  if (!field || fileType.value !== 'pdf') {
    return;
  }
  ...
  const existingRaw = previewDateInputs.value[key];
  const existingPreview = normalizeFieldValue(previewFieldValues.value[key] || '');
  if (existingRaw || existingPreview) {
    return;
  }

  setPreviewDateValue(getBangkokTodayDateInputValue(), field);
}

function handlePreviewDateChange(event: Event): void {
  ...
  setPreviewDateValue(String(input.value || '').trim(), field);
}

function clearPreviewDateValue(): void {
  const field = selectedField.value;
  if (!field) {
    return;
  }

  setPreviewDateValue('', field);
}
...
const canTypePreviewValue = computed<boolean>(() => {
  ...
  if (fieldType === 'signature' || fieldType === 'icon' || fieldType === 'date' || isCheckboxField(selectedField.value)) {
    return false;
  }
...
async function refreshPreviewPdf(): Promise<void> {
  ...
  const { requestValues, overlayValues } = buildPreviewSnapshots();
  const requestSignature = buildPreviewRequestSignature(requestValues);
  ...
  formData.append('fields', JSON.stringify(
    placedFields.value.map((field: FieldInstance) => ({
      ...field,
      sampleValue: requestValues[getPreviewFieldKey(field)] || '',
  ...
  try {
    ...
    previewSyncedFieldValues.value = overlayValues;
...
function resetVisibilityRulesAfterFieldRemoval(removedField: FieldInstance | null | undefined): void {
  ...
  for (const field of placedFields.value) {
    ...
    delete previewDateInputs.value[getPreviewFieldKey(field)];
...
watch([uploadedFile, fileType, previewFieldValues, previewDateInputs, isPreviewOutputEnabled], () => {
  schedulePreviewRefresh();
...
watch(
  ...
  (newField: FieldInstance | null): void => {
    if (newField && typeof newField === 'object') {
      ...
      ensurePreviewDatePrefill(newField);
    }
  },
  { deep: true },
);
      ...
      <section class="flex-1 relative overflow-hidden flex flex-col bg-gray-100">
        ...
        <div v-if="isPreviewOutputEnabled && uploadedFile && fileType === 'pdf'" class="min-h-12 bg-white border-b border-gray-200 px-4 py-2 flex items-center shrink-0 gap-3">
          ...
          <template v-else>
            ...
            <template v-if="isPreviewDateField">
              <input
                :value="selectedFieldPreviewDateInput"
                type="date"
                ...
                @input="handlePreviewDateChange"
              >
              <span class="text-[10px] text-gray-400 shrink-0">DD/MM/YYYY</span>
              <UButton
                size="xs"
                color="neutral"
                variant="ghost"
                :disabled="!selectedFieldPreviewDateInput"
                @click="clearPreviewDateValue"
              >
                {{ t('adminTemplates.create.actions.clear') }}
              </UButton>
            </template>

            <template v-else-if="canTypePreviewValue">
              <input
