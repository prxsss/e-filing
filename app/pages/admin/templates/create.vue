...
async function handleSaveFieldDefaultsFromToolbar(payload: { fieldId: number | string; defaults: any }): Promise<void> {
  ...
  const requestBody = {
    ...
    isAutoGenerated: ['date', 'time'].includes(typeLower)
      ...
      : false,
    dropdownConfig: typeLower === 'dropdown'
      ? normalizeDropdownConfig(
          payload.defaults?.dropdownConfig
          ?? payload.defaults?.dropdown_config
          ?? (fieldDefinition as any).dropdownConfig
          ?? (fieldDefinition as any).dropdown_config
          ?? (selectedFieldData as any)?.dropdownConfig
          ?? (selectedFieldData as any)?.dropdown_config,
        )
      : null,
  };
...
function addFieldToPreview(fieldToAdd: Field): void {
  ...
  const autoGeneratedFlag = normalizeAutoGenerateFlag(fieldToAdd);
  ...
  for (let i = 0; i < amount; i++) {
    const newFieldInstance = {
      ...
      isAutoGenerate: autoGeneratedFlag,
      isAutoGenerated: autoGeneratedFlag,
    ...
    if (i === amount - 1) {
      selectedFieldInstanceIds.value = [newFieldInstance.instanceId];
    }
  }
...
function handleKeyDown(event: KeyboardEvent): void {
  ...
  // Physical KeyC/KeyV so copy/paste works with Thai (and other) keyboard layouts — not event.key ('แ', 'อ', …).
  ...
  <div class="h-screen flex flex-col overflow-hidden">
    ...
    <div v-if="currentWizardStep === 1" class="flex-1 flex overflow-hidden">
      ...
      <section class="flex-1 relative overflow-hidden flex flex-col bg-gray-100">
        <!-- Canvas toolbar (page info | centered field toolbar | zoom) — fixed height so layout does not jump when field-toolbar mounts -->
