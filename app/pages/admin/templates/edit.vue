...
const previewRelevantFieldsSignature = computed<string>(() => {
  const rows: string[] = [];
  ...
  for (const field of placedFields.value) {
    ...
    const previewValue = normalizeFieldValue(previewFieldValues.value[key] || '');
    const hasPreviewValue = previewValue.length > 0;
    const isAutoPreviewField = isAutoGeneratedDateOrTimeField(field);

    if (!hasPreviewValue && !isAutoPreviewField) {
      continue;
    }
    ...
    rows.push([
      ...
      String(field.normalizedX ?? field.x ?? ''),
      String(field.normalizedY ?? field.y ?? ''),
      String(field.normalizedWidth ?? field.width ?? ''),
      String(field.normalizedHeight ?? field.height ?? ''),
      String(field.fontSize ?? ''),
      String(field.fontFamily ?? ''),
      String(field.fontWeight ?? ''),
      String(field.fontStyle ?? ''),
      String(field.textDecoration ?? ''),
      String(field.textAlign ?? ''),
      String(field.letterSpacing ?? ''),
      String(field.lineHeight ?? ''),
      String(field.maxLength ?? field.max_length ?? ''),
      previewValue,
    ].join('|'));
  }
...
watch([uploadedFile, fileType, previewRelevantFieldsSignature], () => {
  schedulePreviewRefresh();
}, { immediate: true });
  ...
  <div class="h-screen flex flex-col overflow-hidden">
    ...
    <div v-if="currentWizardStep === 1" class="flex-1 flex overflow-hidden">
      ...
      <section class="flex-1 relative overflow-hidden flex flex-col bg-gray-100">
        ...
        <div v-if="isPreviewOutputEnabled && uploadedFile && fileType === 'pdf' && selectedField && canTypePreviewValue" class="h-12 bg-white border-b border-gray-200 px-4 flex items-center shrink-0 gap-3">
