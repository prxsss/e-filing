...
definePageMeta({
  title: 'deanDelegatedToSign',
  middleware: ['permission', 'dean-delegation-access'],
...
const { data, status, refresh } = await useFetch<{ success: boolean; data: SigningTask[] }>(
  '/api/requests/for-signing-dean-delegation',
);
...
const columns = computed<any[]>(() => [
  ...
  {
    id: 'studentName',
    header: t('signerToSign.table.studentName'),
    cell: (ctx: any) => {
    ...
    <div class="flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold text-slate-800">
          {{ $t('deanDelegatedToSign') }}
        </h1>
        <p class="text-sm text-slate-500 mt-1">
          {{ $t('deanDelegatedToSignDescription') }}
        </p>
      </div>
      <UButton
    ...
    <UCard>
      ...
      <div v-if="sortedTasks.length === 0 && status !== 'pending'" class="py-12 text-center">
        ...
        <h3 class="font-semibold text-slate-800 mb-2">
          {{ $t('deanDelegatedToSignEmptyTitle') }}
        </h3>
        <p class="text-sm text-slate-500">
          {{ $t('deanDelegatedToSignEmptyDescription') }}
        </p>
      </div>
