...
const hasSignerFillableFields = computed(() => signerFillableFields.value.length > 0);
...
async function refreshPreviewPdf() {
  ...
  if (!baseUrl || !hasSignerFillableFields.value) {
  ...
  const fieldValueSnapshot = Object.fromEntries(
    templatePlacedFields.value.map(field => [getFieldValueKey(field), resolveCurrentFieldValue(field)]),
  );
  ...
  try {
    ...
    formData.append('fields', JSON.stringify(
      templatePlacedFields.value.map((field: any) => ({
        ...field,
...
function schedulePreviewRefresh() {
  clearPreviewTimer();
  if (!hasSignerFillableFields.value || !signingStatus.value?.filledDocumentUrl)
    return;
...
async function saveSignerFieldValues(): Promise<boolean> {
  if (!hasSignerFillableFields.value)
    return true;
          ...
          <div v-else class="divide-y divide-slate-100">
            ...
            >
              ...
              <div class="flex items-center gap-2 shrink-0">
                <UButton v-if="att.fileUrl" size="xs" variant="soft" color="primary" icon="i-lucide-eye" @click="openPreview(att)">
                  ดูไฟล์
                </UButton>
                <UButton v-if="att.fileUrl" size="xs" variant="outline" color="neutral" icon="i-lucide-external-link" :to="att.fileUrl" target="_blank">
                  เปิดใหม่
                </UButton>
              </div>
            </div>
          ...
          <template #body>
            <div v-if="previewAttachment?.fileUrl" class="w-full">
              ...
              <div v-else class="text-center py-12 text-slate-500">
                ...
                <UButton :to="previewAttachment.fileUrl" target="_blank" color="primary" variant="soft" icon="i-lucide-download" class="mt-3">
                  ดาวน์โหลดไฟล์
                </UButton>
              </div>
            </div>
        ...
        <UCard v-if="signingStatus.pendingStep">
          <template #header>
            ...
            <p class="text-sm text-slate-500 mt-1">
              <template v-if="hasSignerFillableFields && hasSignatureField">
                กรอกข้อมูลในช่องด้านล่าง แล้ววาดลายเซ็นและกดยืนยัน
              </template>
              <template v-else-if="hasSignerFillableFields">
          ...
          <div class="space-y-6">
            ...
            <div v-if="hasSignerFillableFields" class="space-y-4">
              ...
              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div
                  v-for="field in visibleSignerFillableFields"
