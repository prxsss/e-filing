...
function normalizeSessionFieldBinding(value: unknown): string {
  const rawValue = String(value ?? '').trim();
  return allowedSessionFieldBindings.includes(rawValue) ? rawValue : 'none';
}
...
const supportsSessionFieldBinding = computed(() => ['text', 'number', 'checkbox'].includes(selectedFieldType.value));

const sessionBindingOptions = computed(() => {
  if (selectedFieldType.value === 'checkbox') {
    return [
      { value: 'none', label: t('adminTemplates.create.fieldModal.sessionBinding.none') },
      { value: 'titleThAutoChecked', label: t('adminTemplates.create.fieldModal.sessionBinding.titleThAutoChecked') },
    ];
  }

  return [
    { value: 'none', label: t('adminTemplates.create.fieldModal.sessionBinding.none') },
    { value: 'studentName', label: t('adminTemplates.create.fieldModal.sessionBinding.studentName') },
    { value: 'studentId', label: t('adminTemplates.create.fieldModal.sessionBinding.studentId') },
    { value: 'studentYearCurrent', label: t('adminTemplates.create.fieldModal.sessionBinding.studentYearCurrent') },
    { value: 'facultyNameTh', label: t('adminTemplates.create.fieldModal.sessionBinding.facultyNameTh') },
    { value: 'departmentNameTh', label: t('adminTemplates.create.fieldModal.sessionBinding.departmentNameTh') },
    { value: 'departmentCode', label: t('adminTemplates.create.fieldModal.sessionBinding.departmentCode') },
  ];
});
  ...
  () => {
    ...
    if (newField) {
      ...
      localField.value = {
        ...
        sessionField: normalizeSessionFieldBinding(newField.sessionField ?? newField.session_field),
...
watch(supportsSessionFieldBinding, (enabled) => {
  if (enabled) {
    localField.value.sessionField = normalizeSessionFieldBinding(localField.value.sessionField);
    return;
  }

  localField.value.sessionField = 'none';
});
...
function onPropertyChange() {
  ...
  const commonStyleUpdates = {
    ...
    visibilityRule: localField.value.conditionalEnabled
      ...
      : null,
    sessionField: supportsSessionFieldBinding.value
      ? normalizeSessionFieldBinding(localField.value.sessionField)
      : null,
    session_field: supportsSessionFieldBinding.value
      ? normalizeSessionFieldBinding(localField.value.sessionField)
      : null,
...
<template>
  <div class="field-toolbar-wrapper bg-white shadow-sm border border-gray-200 rounded-lg px-3 py-1.5 mx-auto">
    <div class="field-toolbar-inline">
      ...
      <template v-if="supportsSessionFieldBinding">
        <div class="h-5 w-px bg-gray-200" />
        ...
        >
          <template #default="{ open }">
            <UTooltip :text="t('adminTemplates.create.fieldModal.labels.autoFillBinding')" :popper="{ placement: 'top' }">
              <button
                type="button"
                class="toolbar-fmt-btn"
                :class="{ active: open }"
                aria-haspopup="dialog"
                :aria-expanded="open"
                :aria-label="t('adminTemplates.create.fieldModal.labels.autoFillBinding')"
              >
          ...
          <template #content>
            <div class="spacing-popover-panel session-binding-panel">
              <div class="spacing-popover-section">
                <label class="spacing-popover-label">{{ t('adminTemplates.create.fieldModal.labels.autoFillBinding') }}</label>
                <select
                  v-model="localField.sessionField"
                  class="toolbar-select session-binding-select"
                  @change="onPropertyChange"
                >
                  <option
                    v-for="option in sessionBindingOptions"
                    :key="option.value"
                    :value="option.value"
                  >
                    {{ option.label }}
