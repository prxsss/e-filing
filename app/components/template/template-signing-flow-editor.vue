...
function isAutoGeneratedField(field: FieldInstance): boolean {
  if (getFieldType(field) === 'time') {
    return true;
  }

  return field.isAutoGenerate === true || (field as any).isAutoGenerated === true || (field as any).is_auto_generated === true;
}

const unassignedFields = computed<FieldInstance[]>(() => {
  return props.placedFields.filter(f => !f.signerStepId && !isAutoGeneratedField(f));
});

const autoGenerateFields = computed<FieldInstance[]>(() => {
  return props.placedFields.filter(f => isAutoGeneratedField(f));
});
...
function assignFieldToStep(fieldInstanceId: string): void {
  ...
  if (!targetField || isAutoGeneratedField(targetField))
    return;
